    ...
    if coordinates is None or 0:
        ...
        for i in range(0, len(coordinates)):
            for j in range(i+1, len(coordinates)):
                val = 0
                for dimen in range(0, 3): # x,y,z
                    val += (coordinates[i,1,dimen] - coordinates[j,1,dimen])**2
...
def kmeans(coords, n_clusters, splitting, **kwargs):
    #if westpa.rc.sim_manager.segments is not None:
    #    with open('/home/jml230/git11/westpa2_tutorials/tutorial-3.1/seg.pickle', 'wb') as fo:
    #        pickle.dump(westpa.rc.sim_manager.segments, fo)
...
def dist_matrix(coords, n_clusters, splitting, **kwargs):
    ...
    ----------
    coords : array-like
        An array with all the pcoordinates of the final frame. It's a default input.
